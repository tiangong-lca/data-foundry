...
export interface FixtureRecordList extends Array<FixtureRecord> {
  find(
    predicate: (value: FixtureRecord, index: number, obj: FixtureRecord[]) => unknown,
  ): FixtureRecord;
  at(index: number): FixtureRecord;
}

export interface FixtureStringList extends Array<string> {
  find(predicate: (value: string, index: number, obj: string[]) => unknown): string;
...
export interface FixtureRecord {
  ...
  operations: FixtureRecordList;
  actions: FixtureRecordList;
  failed_checks: FixtureStringList;
  descriptions: FixtureRecordList;
  amount_scaling_requirements: FixtureRecordList;
  blocked_units: FixtureStringList;
  rows: FixtureRecordList;
  allowed_resolution_modes: string[];
  required_context_file_patterns: string[];
  closes_action_items: Array<string & { code: string }>;
  detected_segments: string[];
  context: FixtureRecord;
  selection: FixtureRecord;
  selected_scopes: FixtureRecord;
  support_identity_cache: FixtureRecord;
  algebra: FixtureRecord;
  execution_requirements: FixtureRecord;
  input_artifacts: FixtureRecord;
  output_artifacts: FixtureRecordList & FixtureRecord;
  input_refs: FixtureRecord;
  outcome: FixtureRecord;
  output: FixtureRecord;
  attempt_state: FixtureRecord;
  resolution: FixtureRecord;
  selected_candidate: FixtureRecord;
  value: FixtureRecord;
  mode: string;
  used_context_kinds: string[];
  skipped_tasks: FixtureRecordList;
  entries: FixtureRecordList;
  entity: FixtureRecord;
  owner_snapshot_receipt: FixtureRecord;
  old: FixtureRecord;
  candidate: FixtureRecord;
  dispositions: FixtureRecordList;
  validation_describe: FixtureRecord;
  protocols: string[];
  summary: FixtureRecord;
  import: FixtureRecord;
  rust_contract: FixtureRecord;
  flowProperty: FixtureRecord;
  name: FixtureRecord;
  raw_sha256: string;
  ...
  terminal_success: FixtureRecord;
  scope_ids: string[];
  old_payload: FixtureRecord;
  baseName: FixtureRecord;
  same_amount_vector_scopes: number;
  same_skeleton_scopes: number;
  same_skeleton_only_scopes: number;
  canonical_support_id: string;
  source_name: string;
  ...
  "common:shortDescription": FixtureRecordList & FixtureRecord;
...
export interface FixtureJsonLineList extends Array<FixtureJsonLine> {
  find(
    predicate: (value: FixtureJsonLine, index: number, obj: FixtureJsonLine[]) => unknown,
  ): FixtureJsonLine;
  at(index: number): FixtureJsonLine;
}
...
export function readJsonLines(filePath: string): FixtureJsonLineList {
  const text = fs.readFileSync(filePath, "utf8").trim();
  return (
    text ? text.split(/\r?\n/u).map((line) => JSON.parse(line) as FixtureJsonLine) : []
  ) as FixtureJsonLineList;
}
