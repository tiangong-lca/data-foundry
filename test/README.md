...
OIDC diagnostics share the publisher's existing response checks and expose only fixed field kinds, boolean validation results and numeric timing differences. Tests reject wrong diagnostic job/event/repository/runner/source bindings before any request, exercise rejected token/date/lifetime fields, and prove that credentials and arbitrary response values never appear in the report. The preparation command suite rejects ordinary execution and caller arguments without exposing supplied values. These fixtures verify the diagnostic boundary; only an explicit real workflow dispatch can identify a production response mismatch, and it cannot publish or replace full release qualification.
