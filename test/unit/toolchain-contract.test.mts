...
test("Foundry pins the published CLI runtime and high-risk audit closure", () => {
  assert.equal(packageJson.dependencies?.["@tiangong-lca/cli"], "0.1.13");
  const runtimeSource = readText("scripts/lib/foundry-runtime-utils.ts");
  assert.match(runtimeSource, /tiangongLcaCliPackageVersion\s*=\s*"0\.1\.13"/u);
  ...
  assert.match(workspace, /-\s+["']?@tiangong-lca\/cli@0\.1\.13["']?/u);
