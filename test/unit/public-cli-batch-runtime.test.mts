...
test("Foundry consumes the published CLI 0.1.12 batch, run-lock, and auth parser exports", async () => {
  const installed = resolveInstalledTiangongLcaCliPackage();
  assert.equal(installed.packageVersion, "0.1.12");
  assert.equal(installed.packageSpec, "@tiangong-lca/cli@0.1.12");
  ...
  assert.equal(parseAuthIdentityReceipt(testAuthIdentityReceipt()).cli.package_version, "0.1.12");
