...
test("installed CLI resolution and runtime command precedence stay package-pinned and shell-safe", () => {
  withTempRoot("foundry-runtime-command", (root) => {
    ...
    assert.equal(installed.packageVersion, "0.1.1");
    assert.equal(installed.packageSpec, "@tiangong-lca/cli@0.1.1");
    ...
    assert.equal(defaultCommand.package, "@tiangong-lca/cli@0.1.1");
    assert.equal(defaultCommand.package_version, "0.1.1");
