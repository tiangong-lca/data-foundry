...
function receipt(capturedAtUtc: string, packageVersion = "0.1.11") {
...
test(
  "production contact case direct entry is symlink-safe",
...
test(
  "production contact case default pinned runtime completes offline gates before env access",
  { skip: process.platform === "win32" },
  async () => {
...
test(
  "production contact case executes one bounded owner-draft mutation and unique readback",
  { skip: process.platform === "win32" },
  async () => {
    ...
    let receiptPackageVersion = "0.1.11";
    ...
    try {
      ...
      const runtimeDriftOutDir = path.join(root, "runtime-drift-before-commit-output");
      authCallsThisRun = 0;
      let runtimeVerifications = 0;
      const commitsBeforeRuntimeDrift = commitDispatches;
      await assert.rejects(
        runProductionContactDraftCase(
          {
            envFile,
          ...
          },
          {
            ...
            prepareRuntimeSnapshot: () => ({
              ...
              verifyCurrent: () => {
                ...
                if (runtimeVerifications === 15) throw new Error("fixture runtime drift");
              },
            }),
            spawnImpl: successfulSpawn,
          },
        ),
        /Runtime evidence drifted at commit-contact-draft/u,
      );
      assert.equal(commitDispatches, commitsBeforeRuntimeDrift);
      assert.equal(
        readJson(path.join(runtimeDriftOutDir, "case-failure.json")).mutation_dispatch_count,
        0,
      );
      ...
      assert.equal(
        readJson(path.join(driftOutDir, "case-failure.json")).mutation_dispatch_count,
        0,
      );
