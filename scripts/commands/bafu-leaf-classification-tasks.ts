...
function loadProcessCategorySchema(schemaPath: unknown): BafuLeafCategorySchema {
  ...
  if (!resolved || !fs.existsSync(resolved)) {
    throw new Error(
      "--process-category-schema is required unless the installed @tiangong-lca/cli@0.1.9 process category schema exists.",
    );
  }
...
function loadFlowProductCategorySchema(schemaPath: unknown): BafuLeafCategorySchema {
  ...
  if (!resolved || !fs.existsSync(resolved)) {
    throw new Error(
      "--flow-product-category-schema is required unless the installed @tiangong-lca/cli@0.1.9 flow product category schema exists.",
    );
  }
