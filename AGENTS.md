...
lastReviewedNote: "Reviewed for Issue #69: strict datetime fail-close and repository-anchored managed output ownership prevent symlink/marker deletion escapes without remote-write authority."
---
...
- `prewrite-cleanup.ts` is the typed deterministic evidence-cleanup boundary. Targeted datetime metadata requires full seconds plus a timezone, exact Gregorian calendar/clock validity, and an offset within the previously accepted `00:00..23:59` range before UTC normalization. It plans every row first; any invalid value emits ordered blockers and prevents all normalization, annual-supply sentinel, source proof, trace externalization, namespace, redaction, and cleaned-row output for the batch. Valid offsets normalize only when UTC remains in the four-digit year grammar; year-boundary values keep exact source bytes. `bundle-sample-utils.ts` must preserve invalid or timezone-less timestamps unchanged for this gate rather than rolling them forward or inventing `Z`. A blocked rerun invalidates stale cleaned/downstream artifacts only when `managed-output-safety.ts` proves a strict realpath descendant of the repository's physical `.foundry/workspaces` root. The shared root itself, symlinked root components, child escapes, custom/explicit paths, replaced artifacts, and legacy/forged markers never gain deletion authority; retained stale defaults are reported before the blocked import ledger is recorded. Exchange arrays and object insertion order remain part of proof hashes; circular/unserializable trace input throws before source evidence is deleted.
