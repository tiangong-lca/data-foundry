...
lastReviewedNote: "Reviewed for Issue #70: all five public command owners are thin; the BAFU facade delegates to an explicit composition root and bounded semantic modules over CLI 0.1.3."
---
...
- The owner CLI is installed as the exact project dependency `@tiangong-lca/cli@0.1.3` and invoked with `pnpm exec tiangong-lca`; Foundry runtime adapters and the account wrapper resolve the same installed package manifest and bin directly. Do not use `dlx` or `@latest` for the owner CLI. The external `skills@latest` package remains intentionally floating and its resolved upstream ref must still be recorded in task evidence.
- Foundry imports reusable CLI primitives only from supported `./command-spec`, `./batch`, and `./auth-identity-receipt` subpaths. Private `dist/src/**`, remote receipt runners, and CLI test internals are not a compatibility surface. `test/fixtures/auth-identity-receipt.ts` may materialize deterministic test-only receipt bytes, but production parsing remains the public CLI parser. The 1,440-minute release-age policy stays global; only exact provenance-verified `@tiangong-lca/cli@0.1.3` is allowlisted while this freshly published dependency is consumed.
...
- `foundry-runtime-utils.ts` is the typed shared runtime boundary. It preserves the exact installed `@tiangong-lca/cli@0.1.3` package/bin/schema resolution, environment override command prefix, synchronous text/JSON/JSONL and portable path helpers, task frontmatter/scalar parsing, explicit env-file precedence, stage/blocker envelopes, local CLI subprocess JSON parsing, hashes and deterministic UUIDs. `foundry-runtime-paths.ts` owns trusted source/emitted root and entry discovery; `foundry-runtime-environment.ts` owns credential-free child isolation. Tests use explicit temporary env files or the internal filesystem-env-disabled policy and never read repository credentials.
...
- BAFU category-map projection is fail-closed over the complete emitted manual-review closure, including decision conflicts, invalid codes, missing task-bound context, and incomplete rows that no current task references. Any such row makes the top-level report `completed_with_manual_review`, adds a source/reason/artifact-bound blocker, and makes the public command exit nonzero. A fully resolved projection retains the exact prior report bytes and zero exit.
- `scripts/lib/batch-orchestration/cli-bounded-batch-runner.ts` is the sole Foundry adapter around public CLI batch/run-lock primitives. It owns canonical run/item contracts, the cross-process run lock, bounded claims, family exclusive keys, pause-before-claim, stop-after-blocked, and in-flight drain. The command owner supplies scope execution, retryable failure ledger projection, best-effort cache capping, family facts, and final report bytes; do not recreate a worker counter or `Promise.all` claim loop in Foundry.
