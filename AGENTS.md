...
- Keep `contracts/supabase-consumer-manifest.v3.json` permanently candidate/non-authorizing. Its exact Git-blob/AST evidence records both direct Supabase reads/auth and indirect CLI/control-plane calls; it never authorizes database freeze, migration, hosted mutation, merge, or deploy.
...
17. Before changing a Supabase-facing runtime, CLI adapter, subprocess helper, credential surface, or agent command surface, run `node scripts/audit-supabase-consumers.mjs --verify`. Regenerate only from an explicit immutable source commit; the delivery HEAD must preserve the same filtered governed Git tree, with only the exact audit tool exempted.
...
- Supabase consumer evidence must map each AST/structured occurrence exactly once to capability, direct/indirect upstream, transport, credential, role, schema/object, signature, ACL, and semantics. Omission, duplication, dynamic bypass, source/delivery drift, upstream substitution, or schema/origin drift must fail closed.
