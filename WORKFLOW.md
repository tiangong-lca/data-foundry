...
lastReviewedNote: "Reviewed for Issue #69: strict cleanup and repository-anchored managed-task invalidation must pass before any cleaned artifact or downstream finalize stage is consumed."
tracker:
  kind: filesystem
  inbox: tasks/inbox
  active: tasks/active
...
Cleanup validates every targeted `common:timeStamp` / `common:dateOfLastRevision` before applying any transform. Accepted values have full seconds and `Z` or an existing-compatible `±HH:MM` offset; Gregorian date, clock, and offset components must round-trip exactly. UTC normalization preserves exact source bytes when conversion would leave the four-digit year grammar. Partial dates, missing timezone, sentinels, non-strings, or impossible components produce `blocked_invalid_datetime_metadata`, a nonzero CLI exit, ordered blocker evidence, and no cleaned rows. A blocked rerun removes stale output only when it is a strict realpath descendant of the repository's physical `.foundry/workspaces` root. The shared root itself, symlinked root components, child symlink escapes, explicit/custom paths, replaced artifacts, and legacy/forged markers cannot grant deletion authority; a retained stale default adds `stale_cleanup_artifact_not_invalidated`. Do not consume `files.cleaned_rows` unless status is `completed`, blockers are empty, and the path is non-null. Post-authoring finalize runs parent cleanup before nested source/contact support finalization; on failure it purges only the same trusted managed-task roots, reports every other stale artifact as a blocker, writes the blocked import ledger, and may not run identity preflight, queue, schema, QA, location audit, dry-run, mutation manifest, or handoff.
