...
lastReviewedNote: "Reviewed for Issue #69: strict cleanup and realpath-bound stale invalidation must pass before any cleaned artifact or downstream finalize stage may be consumed."
tracker:
  kind: filesystem
  inbox: tasks/inbox
  active: tasks/active
...
Cleanup validates every targeted `common:timeStamp` / `common:dateOfLastRevision` before applying any transform. Accepted values have full seconds and `Z` or an existing-compatible `±HH:MM` offset; Gregorian date, clock, and offset components must round-trip exactly. UTC normalization preserves exact source bytes when conversion would leave the four-digit year grammar. Partial dates, missing timezone, sentinels, non-strings, or impossible components produce `blocked_invalid_datetime_metadata`, a nonzero CLI exit, ordered blocker evidence, and no cleaned rows. A blocked rerun removes stale output only when its real path remains under `.foundry/workspaces` or a regular non-symlink marker binds both the same lexical and real output path; a lexical managed path that escapes through a symlink cannot gain ownership. Explicit or otherwise unowned user paths remain intact. Do not consume `files.cleaned_rows` unless status is `completed`, blockers are empty, and the path is non-null. Post-authoring finalize runs parent cleanup before nested source/contact support finalization; on failure it purges exact ownership-proven downstream roots, reports unowned stale artifacts as blockers, writes the blocked import ledger, and may not run identity preflight, queue, schema, QA, location audit, dry-run, mutation manifest, or handoff.
