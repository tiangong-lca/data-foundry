...
Before sealing the handoff capsule, the public workflow rechecks its current identity receipt after local preparation. If that receipt has become stale, it obtains a fresh identity from the same qualified CLI and explicit authentication context. Context or runtime mismatches propagate immediately. Capsule creation still independently revalidates the active grant, task/account binding, lineage and evidence; refreshing identity does not extend the grant lifetime or reset an attempt. The 60-second identity freshness bound is unchanged.
