...
`task resume --reference-input <file>` selects `tiangong-foundry.reference-input.v1` separately from semantic input or approval. The CLI and direct JavaScript facade both reject an explicitly supplied empty reference path before task preparation or execution; only an omitted/undefined option means no new selection. The strict descriptor contains `task_id`, `actor_id`, current `rows_manifest_sha256`, `dataset_type`, `qa_reference_rows` (file/SHA-256 pairs), `intent` (one file/SHA-256 pair or null), and `review_files` (file/SHA-256 pairs). At least QA input or an intent is required. QA references apply only to Process; an intent requires its complete independently selected review-file set. Each list is bounded to 128 files, each file to 8 MiB, and the entire selection to 64 MiB. Credential paths, linked files, wrong task/actor, stale manifests, changed bytes and unused/missing review selections fail before snapshot publication.

The existing task artifact transaction records a fresh immutable generation. QA and review bytes are copied exactly; the original descriptor/intent bytes are retained. The derived intent rewrites only review file locators to the independently checked task snapshots. That derived file has its own raw SHA-256 and is the file the qualified CLI verifies. It grants no permission. Identical submissions reuse the current selection. Concurrent replacement is checked under the task lock; prepared, consumed and completed owner scopes cannot replace their reference evidence. Removing the original selected files does not remove the snapshots needed for execution/recovery.

Selection invalidates prior finalization/approval projection through the recorded reference-selection digest. Finalization uses only current indexed snapshots and requires the selected dataset's current row bytes to match its original control binding; changed rows require fresh selection. Process QA receives explicit repeated `--reference-rows-file` arguments. Precommit verification receives exactly the derived `--reference-intent-file`; default canonical-reference and root policies remain owned by the CLI. Its [exact-reference protocol](https://github.com/tiangong-lca/tiangong-cli/blob/main/docs/agents/exact-reference-intent-contract.md) owns consumer/actor/content/review eligibility. Foundry does not infer units, discover sibling files or create a reference exception itself.

Sealing obtains a fresh qualified CLI identity after local handoff preparation and performs complete capsule admission against the current grant and evidence. Before an unconsumed write, the workflow obtains a new current identity between initial rehydration and the final complete admission pass, so prior local work does not age that receipt. Both passes and the existing 60-second identity bound remain enforced before the attempt is consumed.

A passing precommit selection is carried automatically into both handoff CommandSpecs as final-row, intent, precommit and review artifact facts (plus the native contract when selected). The write command receives no unsupported reference flag. Admission rechecks the complete independently selected evidence set. Every readback, including unknown-response recovery with no commit report, requires the same current intent/consumer/review binding and still passes independent root/owner/state/payload checks. Completed-scope checks rehash retained reference facts. Missing or changed evidence never permits a mutation replay or a root-only completion shortcut.
