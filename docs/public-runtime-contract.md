...
| `tiangong-foundry task resume --workspace <path> --task <id> --actor <id> [--semantic-input <file> | --authorization-input <file>] --json` | Continue registered preparation, accept bound semantic input/approval or run qualified read-only identity preflight. Consumed/ambiguous mutation state is readback-only and never replayed. |
