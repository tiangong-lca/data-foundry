...
`task resume --reference-input <file>` selects `tiangong-foundry.reference-input.v1` separately from semantic input or approval. The strict descriptor contains `task_id`, `actor_id`, current `rows_manifest_sha256`, `dataset_type`, `qa_reference_rows` (file/SHA-256 pairs), `intent` (one file/SHA-256 pair or null), and `review_files` (file/SHA-256 pairs). At least QA input or an intent is required. QA references apply only to Process; an intent requires its complete independently selected review-file set. Each list is bounded to 128 files, each file to 8 MiB, and the entire selection to 64 MiB. Credential paths, linked files, wrong task/actor, stale manifests, changed bytes and unused/missing review selections fail before snapshot publication.
