...
4. [x] **foundry** `worldsteel` profile (capped ≤17 mint, full-context on) + docs; `source-semantics.ts` worldsteel branch; widened `post-authoring-finalize.mjs` gate; `worldsteel-batch-import-run.mjs` wrapper (reuses contact `d5710976`) + registrations. New tests green.
