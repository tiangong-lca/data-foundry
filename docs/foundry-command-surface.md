...
lastReviewedNote: "Reviewed for Issue #67 Wave 24: five command owners move to TS7 without changing names, categories, owner exports, help, report bytes, artifacts, exit mapping, profile defaults, or remote-write modes."
---
...
Public command owner paths must be at most two jumps from `scripts/foundry.mjs`. For semantic import-curation commands, prefer owner modules such as `profiles.mjs`, `curation-gate.mjs`, `authoring-packages.ts`, `patch-collect.ts`, `curation-cleanup.mjs`, `trace-summary.mjs`, and `mutation-manifest.mjs` over mechanical part names. Reusable import-curation logic should be exposed through focused workflow facets under `scripts/lib/import-curation/internal/*-workflow.{ts,mjs}` while migration remains incomplete.
