...
Foundry also owns truthful local completion projection for BAFU category-map decisions. Taxonomy vocabularies and semantic AI choices remain schema/decision-owned, but `category-map-projection.ts` must retain and report every supplied manual-review row, including unreferenced rows, and `foundry-command-registry.ts` must return nonzero until that closure is empty. This status/exit guard cannot invent or apply a category choice.
